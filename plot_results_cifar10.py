...
result_dir = os.path.join('results, cifar10')
...
for distance, config in configs.items():
    ...
    for model, xlim_high in config['models']:
        ...
        fig.savefig('results/curves/attack_curves_cifar10_{}_{}.pdf'.format(distance, model), bbox_inches='tight')

configs = {
    'l1': {
        'norm': '$\ell_1$-norm',
        'models': [
            ('Carmon2019', 'Carmon \emph{et al.} -- Wide ResNet 28-10', 30),
            ('Augustin2020', 'Augustin \emph{et al.} -- ResNet-50', 60),
        ],
        'attacks': [
            ('EAD_l1_9x100', r'EAD 9$\times$100', ':', colors[0]),
            ('EAD_l1_9x1000', r'EAD 9$\times$1000', '-', colors[0]),
...
for distance, config in configs.items():
    ...
    fig.savefig('results/curves/attack_curves_cifar10_{}_combined.pdf'.format(distance))
